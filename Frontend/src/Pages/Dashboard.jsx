...
const Dashboard = () => {
  ...
  const loadUserProfile = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setApiData(
          data.user.Apidata && data.user.Apidata.length > 0
            ? data.user.Apidata[0]
            : null
        );
  ...
  const subscribe = async () => {
    ...
    try {
      ...
      if (data.success && data.paymentSessionId) {
        ...
        if (window.Cashfree) {
          ...
          cashfree.checkout({
            ...
            onSuccess: async (paymentData) => {
              ...
              try {
                ...
                const verifyResponse = await fetch(
                  `${API_BASE_URL}/service/payment-success`,
                  {
                    method: "POST",
                    headers: {
                      Authorization: authToken,
                    ...
                    body: JSON.stringify({
                      ...
                      payment_id:
                        paymentData.transaction.paymentId ||
                        paymentData.transaction.txnId,
                ...
                if (verifyData.success) {
                  setPaymentMessage(
                    verifyData.message || "Payment completed successfully!"
                  );
                  showToastMessage(
                  ...
                  setPaymentMessage(
                    verifyData.message || "Error verifying payment status."
                  );
                  showToastMessage("Payment verification failed", "error");
                }
              } catch (error) {
                setPaymentMessage(
                  "Error during payment verification. Please contact support."
                );
            ...
            onFailure: (failData) => {
              ...
              setPaymentMessage(
                failData.message || "Payment failed. Please try again."
              );
              showToastMessage("Payment failed", "error");
            },
            onClose: () => {
              setPaymentMessage(
                "Payment window closed. You can try again when ready."
              );
              setTimeout(() => {
                setShowPaymentModal(false);
              }, 2000);
        ...
        showToastMessage(
          data.message || "Failed to initiate subscription",
          "error"
        );
        setTimeout(() => {
          setShowPaymentModal(false);
      ...
      setPaymentMessage(
        "Error connecting to payment service. Please try again later."
      );
  ...
  const calculateApiUsagePercentage = () => {
    ...
    return Math.min(
      100,
      Math.round((apiData.apicallCount / apiData.limit) * 100)
    );
  };
  ...
  const renderHospitalsList = () => {
    ...
    return hospitals.map((hospital, index) => (
      <div
        ...
        className="bg-gray-900 rounded-lg p-4 mb-4 border w-1/4 border-gray-600 shadow-[0_0_13px_4px] shadow-gray-500"
        data-aos="fade-up"
      >
        ...
        <p className="mt-2">
          ...
          <span className="text-gray-400">{hospital._id}</span>
        </p>
        <p>
  ...
  const renderDoctorsList = () => {
    ...
    if (!hospital || !hospital.Doctors || hospital.Doctors.length === 0) {
      return (
        <p className="text-gray-400">
          No doctors found for this hospital. Add a doctor to get started!
        </p>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="py-6 border-b border-gray-700 flex justify-between items-center">
          <div className="flex items-center text-2xl font-bold text-blue-500">
            <span className="text-4xl">
              🏥<span className="text-3xl text-white">Filt</span>
              <span className="text-3xl text-teal-400">AI</span>
            </span>
          </div>
          {!showAuthForms && (
            <button
              onClick={handleLogout}
              className="text-[20px] px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
            >
              Logout
            </button>
          )}
        ...
        {showAuthForms ? (
          <div
            className="max-w-md mx-auto mt-10 bg-gray-800 p-8 rounded-lg shadow-md"
            data-aos="fade-up"
          >
            ...
            {authMode === "login" && (
              ...
              <div className="mb-6">
                <label htmlFor="loginUsername" className="block mb-2 text-sm font-semibold text-blue-400 tracking-wide">
                  Username
                </label>
                <input
                  type="text"
                  id="loginUsername"
                  className="w-full px-4 py-2 bg-gray-800 border border-blue-500 focus:border-blue-400 focus:ring-2 focus:ring-blue-500/50 rounded-lg text-white placeholder-gray-400 transition-all duration-200 outline-none"
              ...
              <div className="mb-8">
                <label htmlFor="loginPassword" className="block mb-2 text-sm font-semibold text-blue-400 tracking-wide">
                  Password
                </label>
                <input
                  type="password"
              ...
              <button
                type="submit"
                className="w-full py-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-semibold rounded-lg shadow-md transition-all duration-300"
              >
                Login
              </button>
            </form>
          ...
          <div className="py-8">
            ...
            {activeTab === "hospitals" && (
              <div className="bg-gray-800 rounded-lg shadow-md p-6 mb-6">
                ...
                <div className="flex gap-7">{renderHospitalsList()}</div>
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="mt-4">
                      ...
                      <div className="bg-gray-900 text-gray-300 p-3 rounded-md font-mono text-sm overflow-x-auto">
                        <p>
                          http://localhost:3000/api/service/AgentResponse?hospitalId=68026da649ad1088d5a30603
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {doctorForm.availability.map((avail, index) => (
                    <div key={index} className="flex items-center mb-2">
                      <select
                        ...
                        value={avail.day}
                        onChange={(e) =>
                          updateAvailabilityDay(index, e.target.value)
                        }
                      >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateAvailabilitySlots(index, e.target.value)
                        }
                        required
                      />
                      {index === 0 ? (
